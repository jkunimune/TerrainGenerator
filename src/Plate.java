...
public class Plate extends ArrayList<Tile> { // a tectonic plate
  ...
  public void changeCourse(double r) { // r is the amount of randomness
    w = w.plus(new Vector(Math.random(), Math.asin(Math.random()*2-1) + Math.PI/2, Math.random()*2*Math.PI).times(r)).times(1/(1+r));
  }
}
