  ...
  public void plateTechtonics() { // creates mountain ranges, island chains, ocean trenches, and rifts along fault lines
    for (Tile[] thisRow: map) {
      for (Tile thisTil: thisRow) {
        ...
        for (Tile thatTil: adjacentTo(thisTil)) {
          if (!thatTil.equals(thisTil)) { // assuming they are not the same tile
            ...
            if (delTheta.getR() > 4) // if they are farther than 10 tiles apart, ignore them
              break;
...
//  
  public void climateEnhance() {
    for (int i = 0; i < 8; i ++) { // smooths out the climate
      for (Tile[] row: map) {
        for (Tile til: row) {
          ...
          til.rainfall = (int)(.8*til.rainfall + .2*rain/set.size());
          til.temperature = (int)(.8*til.temperature + .2*temp/set.size());
        }
      }
    }
